...
impl HybridCrypto {
    ...
    pub fn encrypt(&self, data: &[u8], recipient_pubkeys: &PublicKeys) -> Result<HybridCiphertext> {
        ...
        let symmetric_key: [u8; 32] = rand::random();
    ...
    pub fn decrypt(&self, ciphertext: &HybridCiphertext) -> Result<Vec<u8>> {
        ...
        // Try RSA first, fallback to Kyber
        let symmetric_key = match self.decrypt_rsa_key(&ciphertext.encrypted_key_rsa) {
            Ok(key) => key,
            Err(_) => self.decrypt_kyber_key(&ciphertext.encrypted_key_kyber)?,
        };
    ...
    fn verify_rsa(message: &[u8], signature_b64: &str, pubkey_pem: &str) -> Result<bool> {
        ...
        match rsa_pub.verify(padding, &message_hash, &signature_bytes) {
            Ok(_) => Ok(true),
            Err(_) => Ok(false),
        }
    }
    ...
    fn verify_dilithium(message: &[u8], signature_b64: &str, pubkey_b64: &str) -> Result<bool> {
        ...
        match dilithium5::verify_detached_signature(&signature, &message_hash, &public_key) {
            Ok(_) => Ok(true),
            Err(_) => Ok(false),
        }
    }
