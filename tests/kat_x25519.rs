//! RFC 7748 §5.2 known-answer tests for X25519.
//!
//! These run against the exact `x25519-dalek` version and feature set this
//! crate depends on, so they catch a wiring/feature-selection mistake that
//! upstream's own tests would not surface here.

use hex_literal::hex;
use x25519_dalek::{PublicKey, StaticSecret};

/// RFC 7748 defines X25519(scalar, u) as scalar multiplication of the
/// Montgomery u-coordinate. `StaticSecret::from(scalar)` clamps per the RFC;
/// `diffie_hellman` computes the scalar mult.
fn x25519(scalar: [u8; 32], u: [u8; 32]) -> [u8; 32] {
    let sk = StaticSecret::from(scalar);
    let point = PublicKey::from(u);
    *sk.diffie_hellman(&point).as_bytes()
}

#[test]
fn rfc7748_vector_1() {
    let scalar = hex!("a546e36bf0527c9d3b16154b82465edd62144c0ac1fc5a18506a2244ba449ac4");
    let u = hex!("e6db6867583030db3594c1a424b15f7c726624ec26b3353b10a903a6d0ab1c4c");
    let expected = hex!("c3da55379de9c6908e94ea4df28d084f32eccf03491c71f754b4075577a28552");
    assert_eq!(x25519(scalar, u), expected);
}

#[test]
fn rfc7748_vector_2() {
    let scalar = hex!("4b66e9d4d1b4673c5ad22691957d6af5c11b6421e0ea01d42ca4169e7918ba0d");
    let u = hex!("e5210f12786811d3f4b7959d0538ae2c31dbe7106fc03c3efc4cd549c715a493");
    let expected = hex!("95cbde9476e8907d7aade45cb4b873f88b595a68799fa152e6f8f7647aac7957");
    assert_eq!(x25519(scalar, u), expected);
}
